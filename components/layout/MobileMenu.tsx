  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Dark overlay */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.45 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="fixed top-0 left-0 z-40 lg:hidden" style={{ width: "100vw", height: "105svh", background: "#000", cursor: "pointer", touchAction: "none" }} onClick={() => onClose()} onTouchMove={(e) => e.preventDefault()} aria-hidden="true" />

          {/* Sliding panel */}
          <aside className="fixed top-0 right-0 z-50 lg:hidden" style={{ width: "100vw", height: "105svh", overflow: "hidden", clipPath: "inset(0)", touchAction: "none" }} onTouchMove={(e) => e.preventDefault()} aria-label="Menu">
