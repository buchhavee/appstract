...
import { normalizeImage, TRANSITION_GALLERY } from "./utils";
...
export function CaseGallery({ images, autoPlaySpeed = 10000, interactionDelay = 3000 }: CaseGalleryProps) {
  ...
  return (
    <div className="relative w-full aspect-9/16 md:aspect-video lg:aspect-auto lg:h-full lg:min-h-[320px] rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.15)]">
      <div className="absolute inset-0 rounded-2xl overflow-hidden bg-neutral-lightest">
        <AnimatePresence initial={false}>
          <motion.div key={currentIndex} className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={TRANSITION_GALLERY}>
            <Image src={currentImage.src} alt={currentImage.title || `Slide ${currentIndex + 1}`} fill className="object-contain" sizes="(max-width: 768px) 100vw, 50vw" priority={currentIndex === 0} />
            <div
        ...
        <div className="absolute bottom-4 left-4 right-4 flex gap-1 z-20">
          {normalizedImages.map((_, index) => (
            <div key={index} className="flex-1 h-1 bg-white/40 rounded-full cursor-pointer overflow-hidden" onClick={() => jumpToImage(index)}>
              <div
